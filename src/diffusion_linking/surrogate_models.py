...
class Gaussian:
    ...
    def evidence(self, n, summary):
        ...
        return -jnp.sum(jax.scipy.special.gammaln(alpha) - jax.scipy.special.gammaln(self.alpha_0) 
                        + self.alpha_0*jnp.log(self.beta_0) - alpha*jnp.log(beta) 
                        + 0.5*jnp.log(self.lam_0) - 0.5*jnp.log(lam) 
...
class BagOfWords:
    ...
    def _post_predictive(self, x, n, counts):
        ...
        return (jax.scipy.special.gammaln(n + 1) - jnp.sum(jax.scipy.special.gammaln(x + 1))
                + jax.scipy.special.gammaln(sum_alpha) - jax.scipy.special.gammaln(sum_alpha + n)
    ...
    def evidence(self, n, summary):
        ...
        return (jax.scipy.special.gammaln(n + 1) - jnp.sum(jax.scipy.special.gammaln(summary + 1))
                + jax.scipy.special.gammaln(sum_alpha) - jax.scipy.special.gammaln(sum_alpha + n)
...
def dirichlet_categorical_logpmf(x, alphas, sum_alpha):
    return (jax.scipy.special.gammaln(sum_alpha) - jax.scipy.special.gammaln(sum_alpha+jnp.sum(x))
