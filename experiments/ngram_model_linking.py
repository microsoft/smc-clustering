...
def compute_lls(entities):
    ...
    """
    return np.array(
        [surrogate.evidence(None, get_ngram_counts(entity.properties["name"], surrogate.n)) for entity in entities]
    ).flatten()
...
candidates = [
    LinkingCandidate(data[0][0], data[6][0], None),
...
print(
    "\n".join(
        [
            f"{candidates[i].entity1.properties['name']}, {candidates[i].entity2.properties['name']}: {scores[i]}"
