...
class SMCClusterer:
    ...
    def compute_scores(self, rng, clusters, force_recompute=False):
        ...
        for cluster_batch in compute_clusters:
            max_size = (
                self.batch_shape[1]
                + max([0, ceil((max([len(c) for c in cluster_batch]) - self.batch_shape[1]) / 8)]) * 8
            )
            ...
            if len(cluster_batch) < self.batch_shape[0]:
                data += [jnp.full((max_size, self.data_dim), float("nan"))] * (
    ...
    def prior(self, cluster_size):
        ...
        return (
            jnp.log(cluster_size / (self.n_obs + self.alpha))
            if cluster_size > 1
            else jnp.log(self.alpha / (self.n_obs + self.alpha))
        )
    ...
    def update_particle(self, rng, new_obs, particle_id, weight):
        ...
        putative_weights = [
            weight
            + self.prior(len(new_cluster))
            - self.prior(self.state.clusters[old_cluster_id].size)
            + self.score_cache[hash(new_cluster)]
            - self.score_cache[old_cluster_id]
            for new_cluster, old_cluster_id in zip(new_clusters[:-1], old_cluster_ids[:-1])
        ]
    ...
    def summary(self):
        ...
        for i, (particle, weight) in enumerate(
            sorted(zip(self.state.particles, self.state.weights), key=lambda c: c[1], reverse=True)
        ):
            weight = jnp.exp(weight) / jnp.sum(jnp.exp(jnp.array(self.state.weights)))
            clusters = sorted(
                [self.state.retrieve_cluster_data(cluster_hash) for cluster_hash in particle],
                key=lambda c: c.shape[0],
                reverse=True,
            )
            print(
                f"Particle {i}, weight {weight:.2g}, {len(clusters)} clusters, {sum([c.shape[0] for c in clusters])} points, {[c.shape[0] for c in clusters]}"
            )
    ...
    def plot_particles(self, n_plots=5, fig_scale=3):
        ...
        for i, (particle, weight) in enumerate(
            sorted(zip(self.state.particles, self.state.weights), key=lambda c: c[1], reverse=True)[:n_plots]
        ):
...
def resample_multinomial(rng, weights, max_particles):
    ...
    resample_idx = jax.random.choice(
        rng, jnp.arange(len(weights)), (max_particles,), replace=True, p=jnp.exp(weights) / jnp.sum(jnp.exp(weights))
    )
