...
def train_tokenizer(
    corpus: Iterable[str], vocabulary: Vocabulary, bpe_vocab_size: int = 800
) -> JsonLMTokenizer:
    """Train a hybrid tokenizer (specials + byte-level BPE for string interiors) from a serialized corpus."""
    return JsonLMTokenizer.train_from_corpus(
        corpus=corpus, vocabulary=vocabulary, bpe_vocab_size=bpe_vocab_size
    )
